...
df = pd.read_csv('date_angle_135.csv')
...
if n_components >= 2:
    ...
    plt.scatter(principal_df.iloc[:, 0], principal_df.iloc[:, 1], edgecolor='k', s=40)
...
# K-means a los datos transformados por PCA
num_clusters = 10
kmeans = KMeans(n_clusters=num_clusters, random_state=0, n_init=10)
kmeans.fit(principal_df)
labels = kmeans.labels_

# Agregar las etiquetas de los clusters al DataFrame de los componentes principales
principal_df['Cluster'] = labels

# Visualizar los clusters en los primeros dos componentes principales
if n_components >= 2:
    plt.figure(figsize=(8, 6))
    scatter = plt.scatter(principal_df.iloc[:, 0], principal_df.iloc[:, 1], c=principal_df['Cluster'], cmap='viridis', edgecolor='k', s=40)
    plt.title('Agrupación de K-medias de características GLCM transformadas por PCA')
    plt.xlabel('Componente Principal 1')
    plt.ylabel('Componente Principal 2')
    plt.legend(handles=scatter.legend_elements()[0], labels=[f'Cluster {i+1}' for i in range(num_clusters)])
    plt.grid()
    plt.show()